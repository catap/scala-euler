...
object p11 extends Problem {
  ...
  val numbers = data.split("\\s").map(_.toInt)

  def get(x: Int, y: Int): Long =
    numbers(x + y * 20)

  lazy val products = for (x <- 0 until 20; y <- 0 until 17) yield {
    Math.max(
      Math.max(get(x, y) * get(x, y + 1) * get(x, y + 2) * get(x, y + 3),
        get(x, y) * get(x + 1, y) * get(x + 2, y) * get(x + 3, y)),
      if (x > 15) 0
      else Math.max(get(x, y) * get(x + 1, y + 1) * get(x + 2, y + 2) * get(x + 3, y + 3),
